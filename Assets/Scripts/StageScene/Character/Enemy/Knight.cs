...
public class Knight : MovingObject
{
    ...
    [HideInInspector] public static Knight instance;
    public float hp;                // 체력
    ...
    */
    ...
    // Knight은 AttemptMove를 Override하여 독자적인 이동 로직을 갖는다.
    ...
    private void ActivateEnemyByStateUpdate()
    {
        ...
        switch (enemyState)
        {
            case EnemyState.Pause:
                // SampleScene인 경우 바로 Idle로 전환

                if (GameManager.instance.currentSceneName == "SampleScene")
                {
    ...
    // NOTE : 함수를 CutsceneManager에서 호출시켜서 해결, 또 무한으로 호출되는 문제가 생겨서 플래그를 추가함.
    ...
    // TODO : Knight의 Idle 상태에서의 움직임 로직이 필요하다.
    ...
    */
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        if (collision.transform != null)
        {
            ...
            {
                if (collision.transform.name.Contains("Laser"))
                {
                    StartCoroutine(Stun());
                }
                else if (collision.transform.name.Contains("Slash"))
                {
                    switch (enemyState)
                    {
                        // 대기 상태에서 공격을 당한 경우
    ...
    private IEnumerator DieCoroutine()
    {
        ...
        Destroy(rageVFX);
        Destroy(gameObject);
        GameManager.instance.gameOveredSceneName = "Stage3Clear";
